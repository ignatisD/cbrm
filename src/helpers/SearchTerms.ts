...
import { pick, uniq, escapeRegExp, cloneDeep } from "lodash";
import {
    IFilter,
    IFilterOp,
...
} from "@interfaces/helpers/SearchTerms";
...
export default class SearchTerms implements ISearchTerms {
    ...
    constructor(limit?: number, st?: ISearchTerms) {
        if (limit) {
     ...
     */
    public static fromScratch(cloneTerms?: ISearchTerms, limit?: number) {
        const st = new SearchTerms();
    ...
    public static clone(st: ISearchTerms): SearchTerms {
        const clonedSt = cloneDeep(st);
        const terms = new SearchTerms();
        return Object.assign(terms, clonedSt);
    }

    public static mimic(st: ISearchTerms): SearchTerms {
        const terms = new SearchTerms();
    ...
    public static handleFilter(key: string, value: any): IFilter {
        ...
        if (key.indexOf("&") === 0) {
            ...
            filter.op = "$regex";
            value = SearchTerms.escapeRegex(value.trim());
            ...
            try {
                ...
                for (let v of value) {
                    ...
                    const secondTerms = new SearchTerms();
    ...
    public static toRequest(terms: ISearchTerms): IRequestTerms {
    ...
    public advancedFilters(toFilter = {}) {
        for (let key in toFilter) {
            ...
            const filter: IFilter = SearchTerms.handleFilter(key, toFilter[key] || "");
            if (!filter) {
                delete toFilter[key];
                continue;
            }
    ...
    public select(fields: string = "") {
        ...
        this.projection = SearchTerms.fixProjection(this.projection);
        return this;
    }
        ...
        Object.keys(popByPath).forEach(path => {
            ...
            temp.populate.forEach(pop => {
                ...
                if (pop.select) {
                    ...
                    finalPop.select = Object.keys(SearchTerms.fixProjection(selectionMap)).join(" ");
                }
                if (pop.populate?.length) {
    ...
    protected _modifyFilter(filter: any, deep: boolean = false): any {
        ...
        if (
            ...
            const value = SearchTerms.escapeRegex(filter["$regex"].toString().trim());
    ...
    public setFilter(key: string, val: any, op: IFilterOp = "$eq") {
        if (/^[\w$]/.test(key)) {
            ...
            const filter = SearchTerms.handleFilter(key, val);
            if (filter) {
                this._setFilter(filter.key, filter.value, filter.op);
            }
        }
    ...
    public searchIn(fields: string[]): ISearchTerms {
        if (!Helpers.isEmpty(this.filters.key) && fields.length) {
            const value = SearchTerms.escapeRegex(this.filters.key.trim());
                    ...
                    if (field.indexOf(key) === 0) { // either exact match or start of multilang
                        if (field.length !== key.length) { // multiLang match
                            this.filters["$or"] = global.languages.map((lang) => {
                                const langVal = SearchTerms.escapeRegex(this.filters[key]);
                                return {
                                    [`${key}.${lang}`]: {
                                        $regex: `.*${langVal}.*`,
                                        $options: "i"
